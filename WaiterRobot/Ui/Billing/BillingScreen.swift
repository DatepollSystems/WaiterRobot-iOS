...
struct BillingScreen: View {
    ...
    @StateObject private var viewModel: ObservableViewModel<BillingState, BillingEffect, BillingViewModel>
    private let table: shared.Table

    init(table: shared.Table) {
        self.table = table
        _viewModel = StateObject(wrappedValue: ObservableViewModel(viewModel: koin.billingVM(table: table)))
    }
